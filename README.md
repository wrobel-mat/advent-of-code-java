...
- `init` - initializes new day by creating `Solution.java` file from [template](src/main/resources/templates/Solution.java) under path `solution/y${year}/d${day}/Solution.java`, it also fetches and caches the puzzle input under path `resources/inputs/${year}/day*.txt`
- `test` - executes the solution and prints out the answer without submitting
